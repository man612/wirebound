...
export default function TrafficChart({ data }: TrafficChartProps): React.JSX.Element {
  const width = 800
  const height = 120
  ...
  const maxVal = useMemo(() => {
    ...
    const max = Math.max(...data.map((point) => Math.max(point.download, point.upload)))
    return max < 100 ? 100 : max
  }, [data])

  const getPoints = (type: 'download' | 'upload'): string => {
    if (data.length < 2) {
      return ''
    }
    ...
    return data
      .map((point, index) => {
        ...
        const y = height - (point[type] / maxVal) * height
        return `${x},${y}`
      })
      .join(' ')
  }
    ...
    <div className="w-full h-[120px] bg-bg-primary border border-border-subtle relative overflow-hidden flex flex-col theme-transition rounded-sm group cursor-crosshair">
      <div className="absolute top-2 right-2 flex gap-3 text-[10px] font-mono z-10 bg-bg-surface/90 px-2 py-1 border border-border-subtle rounded-sm backdrop-blur-sm transition-all duration-300 opacity-80 group-hover:opacity-100">
        ...
        <div className="flex items-center gap-1 text-accent-blue">
          <div
            className="w-1.5 h-1.5 bg-accent-blue rounded-full animate-pulse"
      ...
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="w-full h-full">
        ...
        <line
          ...
          x2={width}
          y2={height / 2}
          className="stroke-border-subtle theme-transition"
          strokeDasharray="2 2"
          strokeWidth="1"
        />
        <line
        ...
        {pointsUp && (
          <>
            <path
              d={`M 0,${height} L ${pointsUp} L ${width},${height} Z`}
        ...
        {pointsDown && (
          <>
            <path
              d={`M 0,${height} L ${pointsDown} L ${width},${height} Z`}

...
export default function Sidebar({ activePage, onNavigate }: SidebarProps) {
  ...
  return (
    <aside className="w-[200px] border-r border-border-subtle bg-bg-primary flex flex-col justify-between pt-8 pb-4 shrink-0 theme-transition">
      <div className="pt-4">
        <div className="text-[10px] font-bold text-text-muted px-4 mb-2 uppercase tracking-wider">Explorer</div>
        <button 
        ...
        >
          <Activity size={14} className={`transition-transform duration-300 ${activePage === 'dashboard' ? 'scale-110' : 'group-hover:rotate-12 group-active:scale-90'}`} /> 
          {t.dashboard}
        </button>
        <button 
        ...
        >
          <SettingsIcon size={14} className={`transition-transform duration-300 ${activePage === 'settings' ? 'rotate-90 scale-110' : 'group-hover:rotate-45 group-active:scale-90'}`} /> 
          {t.settings}
        </button>
      </div>

...
export default function OnboardingScreen({ onComplete }: OnboardingProps): React.JSX.Element {
    ...
    <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center transition-opacity duration-300">
      <div className="w-[460px] bg-bg-surface border border-border-subtle rounded-sm shadow-2xl flex flex-col animate-modal-enter theme-transition">
        ...
        <div className="p-6 space-y-5">
          <p className="text-sm text-text-secondary leading-relaxed">{t.onboardingDesc}</p>
          ...
          <div>
            ...
            <div className="space-y-1.5 border border-border-subtle bg-bg-primary p-2 rounded-sm theme-transition">
              <label
                className={`group flex items-center gap-3 p-2.5 rounded-sm cursor-pointer transition-all duration-200 active:scale-[0.99] ${
                  lang === 'en'
              ...
              >
                ...
                >
                  {lang === 'en' && <span className="w-1.5 h-1.5 bg-accent-blue rounded-full" />}
                </span>
              ...
              >
                ...
                >
                  {lang === 'id' && <span className="w-1.5 h-1.5 bg-accent-blue rounded-full" />}
                </span>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`flex items-center justify-center gap-2 border rounded-sm px-3 py-2 text-sm transition-all duration-200 ${
                  theme === 'light'
                    ? 'bg-accent-blue/10 text-accent-blue border-accent-blue/30'
                    : 'bg-bg-primary text-text-secondary border-border-subtle hover:bg-bg-hover'
                }`}
              >
                <Sun size={14} />
                {t.lightTheme}

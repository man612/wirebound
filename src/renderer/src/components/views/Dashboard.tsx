...
import type { AdbDevice, LogEntry } from '../../../../shared/types'
...
export default function Dashboard({ status, onStart, onStop, devices, trafficData, logs, onClearLogs, t }: DashboardProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-bg-surface theme-transition animate-page-enter">
      ...
      <div className="px-4 py-2 border-b border-border-subtle flex items-center justify-between shrink-0 theme-transition">
        <div className="text-xs text-text-muted">Wirebound <span className="mx-1">/</span> <span className="text-text-primary">{t.dashboard}</span></div>
        <div className="flex items-center gap-2">
           <div className={`w-2 h-2 rounded-full state-morph ${isRunning ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : isConnecting ? 'bg-amber-500' : 'bg-bg-hover'}`}></div>
           <span className="text-xs font-mono state-morph text-text-secondary">
             {isRunning ? t.connected.toUpperCase() : isConnecting ? t.connecting.toUpperCase() : t.disconnected.toUpperCase()}
           </span>
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
        ...
        <div className="border border-border-subtle bg-bg-primary rounded-sm p-4 flex items-center justify-between theme-transition">
          <div>
            <h2 className="text-sm font-semibold text-text-primary transition-colors duration-300">Wirebound Engine</h2>
            <p className="text-xs text-text-secondary mt-1 transition-colors duration-300">{t.onboardingDesc}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            >
              <Laptop size={14} className="transition-transform duration-200 group-hover:-translate-y-0.5 group-active:translate-y-0 text-text-muted group-hover:text-text-primary" /> 
              {t.testDesktopSpeed}
            </button>
            <button 
            ...
            >
              <div className="relative w-[14px] h-[14px]">
                <StopSquare size={14} className={`absolute inset-0 transition-all duration-300 ${isRunning ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-50 rotate-90'}`} />
                <Play size={14} className={`absolute inset-0 transition-all duration-300 ${!isRunning ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-50 -rotate-90'}`} />
              </div>
              {isRunning ? t.stop : isConnecting ? t.connecting : t.start}
            </button>
          </div>
        </div>
        ...
        {/* Traffic Chart */}
        <div>
          <div className="text-[11px] font-bold text-text-muted mb-2 uppercase tracking-wider">{t.trafficMonitor}</div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="text-[11px] font-bold text-text-muted uppercase tracking-wider">{t.dashboard} ({devices.length})</div>
          </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead className="bg-bg-primary border-b border-border-subtle theme-transition">
                <tr>
                  ...
                  <th className="py-2 px-3 font-semibold text-text-muted text-right w-32">{t.actions}</th>
                </tr>
              </thead>
              <tbody>
                {devices.map((dev) => (
                  <tr key={dev.id} className="border-b border-border-subtle/50 hover:bg-bg-primary transition-colors duration-150 group">
                    <td className="py-1.5 px-3">
                      <div className={`w-2 h-2 rounded-full ${dev.status === 'device' ? 'bg-accent-green shadow-[0_0_6px_rgba(22,163,74,0.2)]' : 'bg-bg-hover'}`}></div>
                    </td>
                    <td className="py-1.5 px-3 font-mono text-text-muted">{dev.id}</td>
                    <td className="py-1.5 px-3 text-text-primary font-medium transition-colors group-hover:text-accent-blue">{dev.name || 'Unknown'}</td>
                    <td className="py-1.5 px-3">
                    ...
                    <td className="py-1.5 px-3 text-right">
                      <button 
                        onClick={() => window.api.testSpeedOnDevice(dev.id)}
                      ...
                      >
                        <Zap size={12} className="text-text-muted group-hover/btn:text-accent-yellow transition-all duration-200 group-hover/btn:scale-110 group-hover/btn:rotate-12" /> {t.testDeviceSpeed}
                      </button>
                    </td>
                  </tr>
                ))}
        ...
        <div className="pb-10">
          ...
          <div className="border border-border-subtle rounded-sm bg-bg-primary h-48 overflow-y-auto p-3 font-mono text-[10px] custom-scrollbar">
            {logs.length === 0 ? (
              <div className="h-full flex items-center justify-center text-text-muted italic">{t.gathering}</div>
            ) : (
              <div className="space-y-1">
                {logs.map((log, i) => (
                  <div key={i} className={`flex gap-2 ${log.type === 'stderr' ? 'text-accent-red' : 'text-text-secondary'}`}>
